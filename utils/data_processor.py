...
class DataProcessor:
    ...
    def create_tech_docs_dataset(self) -> List[Dict]:
    ...
    def create_faq_dataset(self) -> List[Dict]:
        """Create FAQ dataset"""
        return [
            {
                "id": "faq_001",
    ...
    def create_news_dataset(self) -> List[Dict]:
    ...
    def populate_collection(self, collection_name: str, documents: List[Dict]):
        ...
        try:
            ...
            print(f"Processing {len(documents)} documents for {collection_name}...")
            ...
            if ids:
                ...
                print(f"Successfully added {len(ids)} documents to {collection_name}")
            else:
    ...
    def setup_all_datasets(self):
        ...
        # Create datasets
        tech_docs = self.create_tech_docs_dataset()
        faq_data = self.create_faq_dataset()
        news_articles = self.create_news_dataset()
        
        # Populate collections
        self.populate_collection("tech_docs", tech_docs)
        self.populate_collection("faq_data", faq_data)
        self.populate_collection("news_articles", news_articles)

...
class FusionRAG:
    def __init__(self):
        ...
        self.query_generator_llm = ChatOpenAI(model="gpt-4o-mini", temperature=0.3)  # For query generation
        self.answer_generator_llm = ChatOpenAI(model="gpt-4o-mini", temperature=0.2)  # For final answer
        self.logs = []
        self.retrievers = {}
                ...
                class CustomRetriever:
                    ...
                    def get_relevant_documents(self, query: str, k: int = 5):
                        try:
                            ...
                            self.logger(f"Retrieval error for query '{query}': {str(e)}")
                            return []
...
Answer:""")
    ...
    def _generate_sub_queries(self, original_query: str) -> List[str]:
        ...
        try:
            ...
            generated_text = query_chain.invoke({"original_query": original_query})
            ...
            self._log(f"Generated {len(all_queries)} different search approaches")
    ...
    def _retrieve_for_queries(self, queries: List[str], dataset: str, k: int = 5) -> List[List]:
        ...
        self._log(f"Searching through {dataset.replace('_', ' ')} knowledge base")
        ...
        for i, query in enumerate(queries, 1):
            ...
            self._log(f"Search approach {i} found {len(documents)} relevant documents")
    ...
    def _reciprocal_rank_fusion(self, results: List[List], k: int = 10) -> List:
        ...
        self._log(f"Found {len(fused_scores)} unique documents across all searches")
        self._log("Top ranked documents based on consistency across search approaches:")
    ...
    def _generate_final_answer(self, question: str, context_docs: List, max_docs: int = 8) -> str:
        ...
        self._log("Analyzing the best documents to create your comprehensive answer")
        ...
        self._log(f"Using the top {len(top_docs)} most relevant documents for answer generation")
    ...
    def process_query(self, query: str, dataset: str, top_k: int = 5, max_context_docs: int = 8) -> Tuple[str, List[str]]:
        ...
        self._log("Starting multi-perspective search and intelligent document fusion")
        ...
        try:
            ...
            if not any(all_results):
                self._log("No relevant documents found across any search approach")
            ...
            final_answer = self._generate_final_answer(query, reranked_docs, max_context_docs)
            
            self._log(f"Completed: Processed {len(queries)} search approaches and analyzed {len(reranked_docs)} documents")

...
class SpeculativeRAG:
    def __init__(self):
        ...
        self.drafter_llm = ChatOpenAI(model="gpt-4o-mini", temperature=0.7)  # For generating drafts
        self.verifier_llm = ChatOpenAI(model="gpt-4o-mini", temperature=0.1)  # Lower temp for consistent scoring
        self.logs = []
        self.retrievers = {}
...
Answer:""")
    ...
    def _multi_perspective_sampling(self, documents: List, k: int = 3) -> List[List]:
        ...
        self._log(f"Sampling: Creating {k} different document perspectives from {len(documents)} total documents")
        ...
        self._log(f"Sampling: Generated {len(subsets)} document subsets with sizes: {[len(s) for s in subsets]}")
        return subsets[:k]
    ...
    def _generate_draft_response(self, query: str, evidence_subset: List[str]) -> str:
        ...
        try:
            evidence_text = "\n\n".join([f"[{i+1}] {doc}" for i, doc in enumerate(evidence_subset)])
    ...
    def _verify_response(self, query: str, evidence_subset: List[str], draft: str, draft_index: int) -> Tuple[float, str]:
        ...
        try:
            ...
            focus_instruction = differentiation_prompts[draft_index % len(differentiation_prompts)]
            ...
            try:
                ...
                self._log(f"Draft {draft_index + 1} focus: {focus_instruction[:50]}...")
                self._log(f"Draft {draft_index + 1} base score: {base_score}, with variation: {final_score:.1f}")
                
                return final_score, score_text
            except:
                # Fallback with variation
                fallback_score = 5.0 + (random.random() - 0.5) * 2.0  # 4.0 to 6.0 range
                return max(1.0, min(10.0, fallback_score)), score_text
                
        except Exception as e:
            self._log(f"Verification error: {str(e)}")
            error_score = 4.0 + (random.random() - 0.5) * 2.0  # 3.0 to 5.0 for errors
    ...
    def process_query(self, query: str, dataset: str) -> Tuple[str, List[str]]:
        ...
        self._log("Starting Speculative RAG pipeline with parallel generation and STRICT verification")
        ...
        try:
            ...
            documents = retriever.get_relevant_documents(query, k=6)  # Get more docs for better sampling
            
            if not documents:
                self._log("No documents retrieved")
                return "I couldn't find relevant documents to answer your question.", self.logs
            
            self._log(f"Retrieved {len(documents)} documents for multi-perspective sampling")
            
            # Step 2: Multi-perspective sampling
            k_perspectives = 3  # Number of different perspectives
            document_subsets = self._multi_perspective_sampling(documents, k=k_perspectives)
            ...
            for i, subset in enumerate(document_subsets):
                ...
                drafts.append(draft)
                self._log(f"Draft {i+1}: Generated response using {len(subset)} documents ({len(draft)} chars)")
            
            # Step 4: STRICT parallel verification
            self._log("Verification: CRITICALLY evaluating quality of each draft response")
            scores = []
            detailed_feedback = []
            
            for i, (draft, subset) in enumerate(zip(drafts, document_subsets)):
                score, feedback = self._verify_response(query, subset, draft, i)
                scores.append(score)
                detailed_feedback.append(feedback)
                self._log(f"Draft {i+1}: STRICT quality score = {score:.1f}/10")
            ...
            self._log(f"Selection: Chose Draft {best_index + 1} with highest score ({best_score:.1f}/10)")
            self._log(f"Completed: Speculative RAG generated {len(drafts)} candidates and selected the best response")

...
class CorrectiveRAG:
    ...
    def _web_search(self, query: str) -> str:
        ...
        try:
            self._log("Web Search: Using OpenAI's web search tool (minimal tokens)")
            
            from openai import OpenAI
            client = OpenAI()
            
            # Create a more focused search query
            focused_query = f"Brief summary: {query[:50]}"  # Limit query length
            ...
            if len(web_content) > 2000:
                ...
                self._log("Web Search: Truncated results to 2000 chars for efficiency")
            
            self._log(f"Web Search: Retrieved {len(web_content)} chars from web")
            return web_content
    ...
    def _grade_documents(self, query: str, documents: List) -> Tuple[List, bool]:
        ...
        for i, doc in enumerate(documents):
            try:
                ...
                if score.binary_score.lower() == "yes":
                    ...
                    self._log(f"Document {i+1}: NOT RELEVANT - will need web search")
        ...
        self._log(f"Document Grading: {len(relevant_docs)}/{len(documents)} relevant ({relevance_ratio:.1%})")
        
        # Decision logic: need web search if too few relevant docs
        if len(relevant_docs) == 0:
            self._log("Decision: NO relevant documents found - web search required")
            need_web_search = True
        elif relevance_ratio < 0.5:
            self._log("Decision: LOW relevance ratio - web search recommended")
            need_web_search = True
        else:
            self._log("Decision: SUFFICIENT relevant documents - no web search needed")
            need_web_search = False
    ...
    def process_query(self, query: str, dataset: str) -> Tuple[str, List[str]]:
        ...
        try:
            ...
            relevant_docs, need_web_search = self._grade_documents(query, documents)
            
            # Step 3: Corrective action if needed
            if need_web_search:
                self._log("Correction: Insufficient relevant documents - applying corrective measures")
                
                # Transform query for better results
                self._log("Query Transformation: Rewriting query for better retrieval")
                improved_query = self.query_rewriter.invoke({"question": query})
                ...
                if relevant_docs:
                    ...
                    self._log("Context: Combined local documents + web search results")
                else:
                    context = web_results
            ...
            self._log("Step 2: Generating corrected response with enhanced context")

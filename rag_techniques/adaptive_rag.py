...
class AdaptiveRAG:
    ...
    def _route_query(self, query: str) -> str:
        ...
        self._log(f"Strategy: Selected '{strategy}' - {strategy_descriptions[strategy]}")
        return strategy
    ...
    def _grade_documents(self, query: str, documents: List) -> List:
        ...
        self._log(f"Document Grading: Evaluating {len(documents)} retrieved documents")
        ...
        self._log(f"Filtering Results: {len(relevant_docs)}/{len(documents)} documents passed relevance check")
    ...
    def _standard_retrieval(self, query: str, dataset: str) -> List:
        ...
        self._log(f"Retrieved {len(docs)} documents using standard similarity search")
        return docs
    ...
    def _multi_retrieval(self, query: str, dataset: str) -> List:
        ...
        # First retrieval
        retriever = self.retrievers[dataset]
        docs1 = retriever.get_relevant_documents(query, k=3)
        ...
        docs2 = retriever.get_relevant_documents(alt_query_prompt, k=2)
        ...
        self._log(f"Combined results: {len(unique_docs)} unique documents after deduplication")
        return unique_docs
    ...
    def process_query(self, query: str, dataset: str) -> Tuple[str, List[str]]:
        ...
        try:
            ...
            if not filtered_docs:
                self._log("Decision: No relevant documents found - trying query rewrite as fallback")
                if strategy != "rewrite":  # Avoid infinite loop
                    improved_query = self._rewrite_query(query)
                    documents = self._standard_retrieval(improved_query, dataset)
                    filtered_docs = self._grade_documents(improved_query, documents)
            
            # Step 5: Generate response
            self._log("Step 2: Generating adaptive response with filtered context")
            ...
            def format_docs(docs):
                ...
                total_chars = sum(len(doc.page_content) for doc in docs if doc.page_content)
                self._log(f"Context: Using {len(docs)} filtered documents ({total_chars} chars)")
                return "\n\n".join(doc.page_content for doc in docs if doc.page_content)
            
            rag_chain = (
                {"context": lambda x: format_docs(filtered_docs), "question": RunnablePassthrough()}
                | prompt
                | self.llm
                | StrOutputParser()
            )
            ...
            # Final summary
            self._log(f"Completed: Adaptive RAG with {strategy} strategy successfully processed query")

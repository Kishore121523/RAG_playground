...
class ConversationalRAG:
                ...
                class CustomRetriever:
                    ...
                    def get_relevant_documents(self, query: str, k: int = 5):
                        try:
                            ...
                            self.logger(f"Retrieved {len(documents)} documents using context-aware search")
                            return documents
    ...
    def _create_contextual_query(self, current_query: str, history: List[Dict]) -> str:
        ...
        if last_exchange:
            ...
            self._log(f"Context injection: '{last_exchange['user'][:60]}...' + '{current_query[:60]}...'")
    ...
    def process_query(self, query: str, dataset: str, conversation_history: List[Dict] = None) -> Tuple[str, List[str]]:
        ...
        try:
            ...
            if conversation_history:
                self._log(f"Memory: Loading {len(conversation_history)} previous exchanges")
                # Show snippet of recent memory
                last_exchange = conversation_history[-1]
                self._log(f"Last context: User asked '{last_exchange['user'][:60]}...'")
            else:
            ...
            self._log("Prompt: Using conversation-aware template with memory integration")
            
            # Create contextual query for better retrieval
            original_query = query
            contextual_query = self._create_contextual_query(query, conversation_history)
            ...
            def format_docs(docs):
                ...
                self._log(f"Context: Prepared {len(docs)} docs ({total_chars} chars) with memory")
                return "\n\n".join(doc.page_content for doc in docs if doc.page_content)
            
            # RAG chain with conversation history
            def retrieve_and_format(inputs):
                self._log("Step 1: Retrieving documents with conversation awareness")
                docs = retriever.get_relevant_documents(contextual_query)
                formatted = format_docs(docs)
                self._log("Step 2: Generating response with memory + retrieved context")
                return formatted
            
            def format_history(inputs):
                formatted = self._format_conversation_history(conversation_history)
                if conversation_history:
                    self._log(f"Memory: Injecting {len(conversation_history)} exchanges into prompt")
                return formatted
            ...
            self._log(f"Completed: Generated contextual response {context_type}")

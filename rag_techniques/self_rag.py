...
class SelfRAG:
    ...
    def _setup_graders(self):
        ...
        self.hallucination_grader_llm = self.llm.with_structured_output(GradeHallucinations)
    ...
    def _grade_documents(self, question: str, documents: List) -> Tuple[List, bool]:
        ...
        self._log(f"Self-Reflection: Grading {len(documents)} retrieved documents for relevance")
        ...
        for i, doc in enumerate(documents):
            try:
                ...
                if score.binary_score.lower() == "yes":
                    ...
                    self._log(f"Document {i+1}: RELEVANT - keeping for generation")
                else:
        ...
        self._log(f"Document filtering: {relevant_count}/{total_docs} documents passed relevance check")
        
        if need_retry:
            self._log("Self-Reflection: Too many documents failed - will trigger query rewrite")
        else:
            self._log("Self-Reflection: Sufficient documents passed - proceeding with generation")
    ...
    def _self_reflect_on_generation(self, question: str, documents: List, generation: str) -> Tuple[bool, bool]:
        ...
        try:
            ...
            if is_grounded:
                self._log("Hallucination check: PASSED - response is grounded in facts")
            else:
                self._log("Hallucination check: FAILED - response contains unsupported claims")
        ...
        try:
            ...
            if addresses_question:
                self._log("Question relevance check: PASSED - response addresses the question")
            else:
                self._log("Question relevance check: FAILED - response doesn't address the question")
    ...
    def process_query(self, query: str, dataset: str) -> Tuple[str, List[str]]:
        ...
        try:
            ...
            for attempt in range(max_retries + 1):
                if attempt > 0:
                    self._log(f"Retry {attempt}: Attempting improved retrieval and generation")
                
                # Step 1: Retrieve documents
                self._log("Step 1: Initial document retrieval")
                documents = retriever.get_relevant_documents(current_query)
                self._log(f"Retrieved {len(documents)} documents from {dataset}")
                
                # Step 2: Self-reflection on documents
                relevant_docs, need_retry = self._grade_documents(current_query, documents)
                ...
                if need_retry and attempt == max_retries and relevant_docs:
                    ...
                    self._log("Self-Reflection: Final attempt - accepting available relevant documents")
                    need_retry = False
                
                if need_retry:
                    if attempt < max_retries:
                        self._log("Self-Reflection: Document quality insufficient - rewriting query for retry")
                        current_query = self.query_rewriter.invoke({"question": current_query})
                        self._log(f"Rewritten query: '{current_query[:60]}...'")
                        continue
                    else:
                        self._log("Self-Reflection: Max retries reached with insufficient documents")
                ...
                generation = self._generate_response(current_query, relevant_docs)
                ...
                if is_grounded and addresses_question:
                    self._log("Self-Reflection: Response quality approved - accepting answer")
                    self._log("Completed: Self-RAG generated high-quality response with self-reflection")
                    return generation, self.logs
                    
                elif attempt < max_retries:
                    if not is_grounded:
                        self._log("Self-Reflection: Response not grounded - retrying generation")
                    if not addresses_question:
                        self._log("Self-Reflection: Response doesn't address question - rewriting query")
                        current_query = self.query_rewriter.invoke({"question": current_query})
                    continue
                else:
                    self._log("Self-Reflection: Max retries reached - returning best available response")
                    self._log("Completed: Self-RAG completed with quality concerns noted")
                    return generation, self.logs

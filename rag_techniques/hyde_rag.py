...
class HydeRAG:
    ...
    def _generate_hypothetical_document(self, query: str, dataset: str) -> str:
        ...
        self._log("HyDE Generation: Creating hypothetical document for improved retrieval")
        
        # Select appropriate template based on dataset
        template = self.hyde_templates.get(dataset, self.hyde_templates["tech_docs"])
    ...
    def _retrieve_with_hyde(self, hypothetical_doc: str, dataset: str) -> List:
        ...
        documents = retriever.get_relevant_documents(hypothetical_doc)
        
        self._log(f"HyDE Retrieval: Found {len(documents)} documents using hypothetical embedding")
    ...
    def process_query(self, query: str, dataset: str) -> Tuple[str, List[str]]:
        ...
        try:
            ...
            hypothetical_doc = self._generate_hypothetical_document(query, dataset)
            
            # Step 2: Retrieve using hypothetical document
            documents = self._retrieve_with_hyde(hypothetical_doc, dataset)
            
            if not documents:
                self._log("Fallback: No documents found with HyDE, trying direct query")
                # Fallback to direct retrieval if HyDE fails
                retriever = self.retrievers[dataset]
                documents = retriever.get_relevant_documents(query)
                self._log(f"Fallback retrieval: Found {len(documents)} documents")
            ...
            def format_docs(docs):
                ...
                self._log(f"Context: Using {len(docs)} HyDE-retrieved documents ({total_chars} chars)")
                return "\n\n".join(doc.page_content for doc in docs if doc.page_content)
            
            rag_chain = (
                {"context": lambda x: format_docs(documents), "question": RunnablePassthrough()}
                | prompt
                | self.llm
                | StrOutputParser()
            )
            ...
            self._log("Completed: HyDE RAG generated response using hypothetical document retrieval")

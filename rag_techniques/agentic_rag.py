...
class AgenticRAG:
    ...
    def _get_chroma_db_path(self):
        """Get ChromaDB path"""
        current_dir = os.path.dirname(os.path.abspath(__file__))
        project_root = os.path.dirname(current_dir)
        return os.path.join(project_root, "utils", "chroma_db")
    ...
    def _setup_retrievers(self):
        ...
        for dataset in datasets:
            try:
                ...
                self.retrievers[dataset] = CustomRetriever(collection, self.embeddings, self._log)
    ...
    def _detect_query_characteristics(self, query: str) -> Dict[str, bool]:
        ...
        # Temporal indicators
        temporal_words = ["recent", "latest", "current", "today", "now", "new", "upcoming", "2024", "2025"]
        has_temporal = any(word in query_lower for word in temporal_words)
        
        # Complexity indicators  
        complexity_words = ["compare", "difference", "vs", "versus", "how do", "what are", "analyze", "evaluate"]
        is_complex = any(word in query_lower for word in complexity_words) or len(query.split()) > 8
    ...
    def _planning_agent(self, query: str, dataset: str) -> Dict[str, Any]:
        ...
        if characteristics["temporal"] and characteristics["cross_domain"]:
            ...
            self._log("Planning: Comprehensive research needed - temporal + cross-domain query")
            steps = ["local_research", "cross_dataset_search", "web_search", "synthesis"]
            
        elif characteristics["cross_domain"]:
            plan_type = "multi_source"
            self._log("Planning: Multi-source research needed - cross-domain query")
            steps = ["local_research", "cross_dataset_search", "synthesis"]
            
        elif characteristics["temporal"]:
            plan_type = "current_focused"
            self._log("Planning: Current information focus - temporal query detected")
            ...
            plan_type = "simple"
            self._log("Planning: Simple query detected - direct retrieval approach")
    ...
    def _research_agent(self, query: str, dataset: str, plan: Dict) -> List[Dict]:
        ...
        self._log("Agent: Research Agent executing comprehensive information gathering")
        
        research_results = []
        
        # Always do primary local research
        if "local_research" in plan["steps"]:
            self._log("Research: Gathering primary information from local database")
            if dataset in self.retrievers:
                retriever = self.retrievers[dataset]
                documents = retriever.get_relevant_documents(query)
                
                if documents:
                    self._log(f"Research: Found {len(documents)} primary documents in {dataset}")
        ...
        if "cross_dataset_search" in plan["steps"]:
            ...
            other_datasets = [d for d in ["tech_docs", "faq_data", "news_articles"] if d != dataset]
            
            for other_dataset in other_datasets:
                if other_dataset in self.retrievers:
                    other_retriever = self.retrievers[other_dataset]
                    other_docs = other_retriever.get_relevant_documents(query, k=2)
                    
                    if other_docs:
                        self._log(f"Research: Found {len(other_docs)} cross-domain documents in {other_dataset}")
                        research_results.append({
                            "source": "cross_domain",
        ...
        # Supplementary search for complex queries
            ...
            for other_dataset in other_datasets[:1]:  # Limited supplementary search
                if other_dataset in self.retrievers:
                    other_retriever = self.retrievers[other_dataset]
                    other_docs = other_retriever.get_relevant_documents(query, k=2)
                    
                    if other_docs:
                        self._log(f"Research: Found {len(other_docs)} supplementary documents in {other_dataset}")
                        research_results.append({
        ...
        if "web_search" in plan["steps"]:
            self._log("Research: Current information needed - requesting web search")
    ...
    def _tool_agent(self, query: str) -> str:
        ...
        try:
            ...
            self._log(f"Tool: Retrieved {len(web_content)} chars from web search")
            return web_content
    ...
    def _synthesis_agent(self, query: str, research_results: List[Dict]) -> str:
        ...
        context_parts = []
        sources_used = []
        source_categories = {"primary": 0, "cross_domain": 0, "supplementary": 0, "web": 0}
        ...
        for result in research_results:
            if "documents" in result:
                ...
                if result["source"] == "primary":
                    context_parts.append(f"Primary source ({result['dataset']}): {docs_text}")
                    source_categories["primary"] += len(result["documents"])
                elif result["source"] == "cross_domain":
                    context_parts.append(f"Cross-domain source ({result['dataset']}): {docs_text}")
                    source_categories["cross_domain"] += len(result["documents"])
                else:
                    context_parts.append(f"Supplementary source ({result['dataset']}): {docs_text}")
                    source_categories["supplementary"] += len(result["documents"])
                
                sources_used.append(result["dataset"])
                
            elif result["source"] == "web":
                context_parts.append(f"Current information: {result['content']}")
        ...
        self._log(f"Synthesis: Combining information from sources: {sources_list}")
...
Answer:"""
    ...
    def process_query(self, query: str, dataset: str) -> Tuple[str, List[str]]:
        ...
        self._log("Starting Agentic RAG pipeline with robust multi-agent coordination")
        ...
        try:
            ...
            self._log(f"Completed: Used {total_docs + web_sources} information pieces from {info_sources} sources ({sources_summary})")

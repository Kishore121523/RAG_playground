            ...
            if technique in self.techniques:
                ...
                if hasattr(technique_instance, 'process_query'):
                    if technique == "RAG with Memory (Conversational)":
                        ...
                        response, logs = technique_instance.process_query(message, dataset, session_history)
                        
                        # Update conversation history
                        session_history.append({"user": message, "assistant": response})
                        self.conversation_history[session_id] = session_history
                    else:
                        # Standard process_query method
                        response, logs = technique_instance.process_query(message, dataset)
                else:
                    # Fallback method name
                    response, logs = technique_instance.generate_response(message, dataset)
                    
            else:
                # Technique not implemented yet
                response = f"🚧 {technique} coming soon..."
                logs = [f"[{datetime.now().strftime('%H:%M:%S')}] INFO: {technique} not implemented yet"]
...
with gr.Blocks(css=custom_css, title="RAG Playground") as demo:
    ...
    with gr.Tabs():
        with gr.TabItem("🔬 RAG Playground"):
            ...
            demo.load(lambda s: f"**Session ID:** `{s}`", inputs=session_id, outputs=session_display)
            ...
            def process_and_update_logs(history, message, technique, dataset, session_id):
                new_message, new_history = rag_playground.get_response(history, message, technique, dataset, session_id)
            ...
            text_submit_btn.click(
                ...
                inputs=[chatbot, input_txt, technique_dropdown, dataset_dropdown, session_id],
